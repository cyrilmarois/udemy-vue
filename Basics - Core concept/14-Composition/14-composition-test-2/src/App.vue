<template>
  ...
  <section>
    <div>Available Funds: {{ wallet.availableFunds }}</div>
    <div>Total Expenses: {{ wallet.currentExpenses }}</div>
    <hr />
...
const remainingFunds = computed( function () {
  console.log( {remainingFunds: wallet.availableFunds - wallet.currentExpenses} )
  return wallet.availableFunds - wallet.currentExpenses;
} );

function addExpense() {
  wallet.currentExpenses += enteredExpense.value;
}
